...
class CurrencyExchangeTest {
  ...
  void 요약_연산을_할_수_있다() {
    ...
    final int sum4 = numbers.stream()
        .collect(reducing(0, n -> n, Integer::sum)); // 초깃값, 변환함수, 합계함수 (한개 인수로도 됨)
    ...
    final IntSummaryStatistics statistics = menu.stream()
  ...
  void 문자열_연결를_할_수_있다() {
    ...
    final String shortMenu3 = menu.stream().map(Dish::getName).collect(reducing(stringJoin))
        .orElse("");
  ...
  void 그룹화를_할_수_있다() {
    ...
    final Map<DishType, Map<CaloricLevel, List<Dish>>> dishesByTypeCaloricLevel = menu.stream()
        .collect(
    ...
    final Map<DishType, Dish> mostCaloricByType2 = menu.stream().collect(
        toMap(Dish::getDishType, Function.identity(),
  ...
  void 분할을_할_수_있다() {
    ...
    final Map<Boolean, Map<Boolean, List<Dish>>> vegetarianDishesByTypeOver500 = menu.stream()
        .collect(
            partitioningBy(Dish::isVegetarian,
